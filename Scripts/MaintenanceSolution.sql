...
SET @BackupDirectory     = 'C:\SQL_Backup'         -- Specify the backup root directory. If no directory is specified, the default backup directory is used.
SET @CleanupTime         = 2400 /2--(просто поделил на 2)        -- Время самоочистки. пример, 500 часов. Time in hours, after which backup files are deleted. If no time is specified, then no backup files are deleted.
SET @OutputFileDirectory = 'C:\SQL_Backup'         -- ТУу лежит файл с сообщениями об ошибках. Specify the output file directory. If no directory is specified, then the SQL Server error log directory is used.
